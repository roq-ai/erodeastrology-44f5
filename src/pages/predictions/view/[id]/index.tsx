...
import { getPredictionById } from 'apiSdk/predictions';
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('prediction', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/predictions/edit/${data?.id}`} passHref legacyBehavior>
              <Button
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              {hasAccess('birth_chart', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  <Text fontSize="lg" fontWeight="bold" as="span">
                    Birth Chart:
                  </Text>
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/birth-charts/view/${data?.birth_chart?.id}`}>
                      {data?.birth_chart?.birth_place}
                    </Link>
                  </Text>
                </Flex>
              )}
